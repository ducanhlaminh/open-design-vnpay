...
function resolveHeadlessConfig(): PackagedConfig {
  ...
  return {
    ...
    kgsUrl: process.env.KGS_URL?.trim() || null,
    kgsAppId: process.env.KGS_APP_ID?.trim() || null,
    kgsTenant: process.env.KGS_TENANT?.trim() || null,
    kgsApiKey: process.env.KGS_API_KEY?.trim() || null,
    mediaUrl: process.env.MEDIA_URL?.trim() || null,
