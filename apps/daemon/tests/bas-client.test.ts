...
test('searchConfluencePages (direct PAT) maps id/title/url/space from the CQL search results', async () => {
  globalThis.fetch = vi.fn(async (url: any) => {
    const u = String(url);
    assert.match(u, /\/rest\/api\/content\/search\?cql=/);
    assert.match(u, /expand=space/);
    return makeRes(
      JSON.stringify({
        results: [
          {
...
test('searchConfluencePages (BAS gateway fallback) maps id/title/space from the tool result', async () => {
  stubFetch((name) => {
