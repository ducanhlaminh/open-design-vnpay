...
//
// Drives the real express routes (registerKgSyncRoutes) over a live socket with a
// faked `pipelines.pullConflict` dep, so the test pins the wire contract — input
// validation, the `{ ok, data }` envelope, and the PLAN_EXPIRED → 409 mapping —
// without booting the whole daemon or a media-service. The classify/resolve/
// TOCTOU logic itself is covered by pull-conflict.test.ts.
