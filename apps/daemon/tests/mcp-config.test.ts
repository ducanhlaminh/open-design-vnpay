...
describe('seedDefaultMcpConfig / removeLegacyBaAgentSeed', () => {
  let dataDir: string;
  ...
  it('removes the ba-agent entry older versions auto-seeded, keeps everything else', async () => {
    await writeMcpConfig(dataDir, {
      servers: [
        {
          id: 'ba-agent',
    ...
    expect(await removeLegacyBaAgentSeed(dataDir)).toBe(true);
    const cfg = await readMcpConfig(dataDir);
    expect(cfg.servers.map((s) => s.id)).toEqual(['only-mine']);
    // Idempotent: second boot finds nothing to do.
    expect(await removeLegacyBaAgentSeed(dataDir)).toBe(false);
  });

  it('is a no-op when there is no config file or no ba-agent entry', async () => {
    expect(await removeLegacyBaAgentSeed(dataDir)).toBe(false);
    await writeMcpConfig(dataDir, {
      servers: [{ id: 'only-mine', transport: 'http', enabled: true, url: 'https://mine.test/mcp/' }],
    });
    expect(await removeLegacyBaAgentSeed(dataDir)).toBe(false);
    const cfg = await readMcpConfig(dataDir);
    expect(cfg.servers.map((s) => s.id)).toEqual(['only-mine']);
