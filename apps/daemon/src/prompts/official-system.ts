...
3. **Plan with TodoWrite.** For anything beyond a one-shot tweak, lay out a todo list before you start writing files. Update it as you go — the user sees your progress live. If your runtime doesn't expose TodoWrite directly (some defer it behind a tool-search step, some headless runs omit it), load it first where you can or just write the plan as a short prose list — never tell the user the tool is unavailable.
