...
export async function saveFeedbackForm(projectId: string, draft: { title: string; workflowId?: string; sections?: FeedbackFormSection[]; questions: FeedbackQuestion[] }, opts: { client?: MediaClient; user?: string } = {}): Promise<FeedbackFormDef> { const errors = validateFormQuestions(draft.questions, draft.sections); if (!nonEmptyString(draft.title)) errors.unshift('title: không được rỗng'); if (draft.workflowId !== undefined && !nonEmptyString(draft.workflowId)) errors.unshift('workflowId: không được rỗng'); if (errors.length) throw new Error(`Form không hợp lệ: ${errors.join('; ')}`); const client = clientOf(opts); const existing = await resolveForms(projectId, client); const version = Math.max(...existing.map((form) => form.version), 1) + 1; const form: FeedbackFormDef = { version, title: draft.title, ...(draft.workflowId ? { workflowId: draft.workflowId } : {}), ...(draft.sections ? { sections: draft.sections } : {}), questions: draft.questions, createdAt: Date.now(), ...(opts.user ? { createdBy: opts.user } : {}) }; await client.uploadFile(projectId, FORMS_DIR, `feedback/forms/v${version}.json`, 'application/json', Buffer.from(`${JSON.stringify(form, null, 2)}\n`, 'utf8')); return form; }
