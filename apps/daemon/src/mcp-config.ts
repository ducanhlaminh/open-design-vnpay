...
function tomlString(value: string): string {
  // TOML basic string: escape `\` and `"` and bytes < 0x20 (rare in
  // command paths / env values, but we cover them to keep the writer
  // honest). Newlines deliberately escape rather than switching to
  // multi-line, so a malformed env value can't break out into a new
  // key/value pair.
  let out = '"';
...
function tomlStringArray(items: string[]): string {
  return '[' + items.map((s) => tomlString(s)).join(', ') + ']';
}
