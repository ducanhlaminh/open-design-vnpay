      ...
      while (true) {
        ...
        while ((idx = buf.indexOf('\n\n')) !== -1) {
          ...
          if (event.event === 'start') {
            ...
            onRunStatus?.('running');
            // Synthesize a `skills_applied` event from the start payload's
            // skill ids before the `status: 'starting'` so the assistant
            // message has the skill context attached at the top of its
            // event list. Emit even when the lists are empty so consumers
            // can distinguish "no skill" from "skill data missing".
            const skillIdsList = Array.isArray(data.skillIds)
              ? data.skillIds.filter(
            ...
            const primarySkill =
              typeof data.skillId === 'string' && data.skillId.length > 0
                ? data.skillId
                : null;
            if (primarySkill || skillIdsList.length > 0) {
