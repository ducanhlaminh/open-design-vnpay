// @ts-nocheck
// In-browser HTML→IR extractor for the web "Copy to Figma" button (apps/web).
//
// The `walkInPage` function below is a VERBATIM copy of
// skills/html-to-figma/assets/extract.cjs's `walkInPage` (lines 25–691). KEEP IN SYNC: it is
// the same proven DOM walker the CLI/skill runs under Playwright. We don't re-port it — we
// `.toString()` it and recreate it inside the artifact iframe via that window's Function
// constructor, so getComputedStyle/getCTM/getBoundingClientRect resolve against the rendered
// document. Then we embed <img>/background images (fetch → base64) in the web context.
//
// Output IR matches @open-design/figma-clip's IRNode contract (the daemon runs irToClip on it).
...
// ---- web glue (not part of the verbatim extractor) ----

async function embedImages(ir, win) {
  const nodes = [];
  (function collect(n) {
  ...
  for (const n of nodes) {
    ...
    delete n.imageUrl;
    try {
      let entry = cache.get(u);
      if (!entry) {
        const abs = new URL(u, win.location.href).href;
        ...
        entry = { data, format: blob.type || "image/png" };
        cache.set(u, entry);
      }
      n.image = { data: entry.data, format: entry.format, scaleMode: "FILL" };
    } catch {
      /* leave imageUrl removed → lib renders a gray placeholder + warns */
    }
  }
}

export interface ExtractIRResult {
  ir: unknown;
  warnings: string[];
}

// Extract the IR from a rendered artifact iframe. `selector` defaults to the body's first
// element child (the artifact root), matching the CLI extractor.
export async function extractIR(
  iframe: HTMLIFrameElement,
  selector?: string | null,
): Promise<ExtractIRResult> {
  const win = iframe.contentWindow;
  if (!win) throw new Error("Không truy cập được nội dung iframe (preview chưa load?)");
  // cross-origin guard: touching the iframe document throws a SecurityError when the artifact
  // is served from a different origin than the web app.
  try {
    void win.document.body;
  } catch {
    throw new Error("Artifact khác origin — không đọc được DOM (cross-origin iframe)");
  }
  // recreate walkInPage inside the iframe's realm so getComputedStyle/getCTM resolve there
  let fn;
  try {
    fn = new win.Function("return (" + walkInPage.toString() + ")")();
  } catch (e) {
    throw new Error("Trình duyệt chặn extractor trong iframe (CSP unsafe-eval?): " + (e && e.message ? e.message : e));
  }
  const result = fn(selector || null);
  if (result && result.error) throw new Error(result.error);
  if (!result || !result.ir) throw new Error("Không trích xuất được IR từ artifact (root rỗng?)");
  await embedImages(result.ir, win);
  return { ir: result.ir, warnings: Array.isArray(result.warnings) ? result.warnings : [] };
}

// Render an artifact HTML string into a throwaway, offscreen, SAME-ORIGIN iframe (srcdoc), then
// extract. This sidesteps the cross-origin preview iframe (the daemon serves the live preview on
// a different origin than the web app, so its DOM is unreadable). `width` should match the
// rendered preview so layout/sizing line up.
export async function extractIRFromHTML(html: string, width = 430): Promise<ExtractIRResult> {
  if (typeof html !== "string" || !html.trim()) throw new Error("Không có nội dung artifact để trích xuất");
  ...
  try {
    ...
    if (!win) throw new Error("Không tạo được iframe trích xuất");
    try {
      await (win.document.fonts?.ready ?? Promise.resolve());
    } catch {
      /* fonts best-effort */
    }
    await new Promise((r) => setTimeout(r, 180)); // let layout settle after fonts
    return await extractIR(tmp);
  } finally {
    tmp.remove();
  }
}
