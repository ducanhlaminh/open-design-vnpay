  ...
  return (
    <div className={`${styles.root} ${wizard ? styles.wizard : ''}`}>
      ...
      <main className={styles.content}>
        ...
        {attachmentSection && <>
          {stageOptionsVisible && <section className={styles.attachments}><h3 className={styles.subheading}>Đính kèm output</h3>{stageOutputs.filter((stage) => stage.files.length).map((stage) => <fieldset className={styles.stageGroup} key={stage.stageId}><legend className={styles.stageName}>{stage.stageName}</legend>{stage.files.map((file) => { const checked = stageFiles.some((item) => item.stageId === stage.stageId && item.sourcePath === file.sourcePath); return <label className={styles.checkOption} key={file.sourcePath}><input type="checkbox" checked={checked} onChange={() => setStageFiles((current) => checked ? current.filter((item) => !(item.stageId === stage.stageId && item.sourcePath === file.sourcePath)) : [...current, { stageId: stage.stageId, sourcePath: file.sourcePath, name: file.name }])} />{file.name}</label>; })}</fieldset>)}</section>}
...
function QuestionField({ instance, answers, context, otherTexts, setAnswer, toggleCheckbox, setOtherTexts }: {
  ...
  return <fieldset className={styles.field}><legend className={styles.label}>{question.label}{question.required && <span className={styles.required}> *</span>}</legend>
    ...
    {(question.type === 'radio' || question.type === 'checkbox') && (options.length ? <div className={styles.options}>{[...options, ...(question.allowOther ? [OTHER] : [])].map((option) => <label className={styles.checkOption} key={option}><input type={question.type === 'radio' ? 'radio' : 'checkbox'} name={key} checked={question.type === 'radio' ? value === option : Array.isArray(value) && value.includes(option)} onChange={() => question.type === 'radio' ? setAnswer(key, option) : toggleCheckbox(key, option)} />{option === OTHER ? 'Khác' : option}</label>)}</div> : <p className={styles.hint}>Không có bước nào trong ngữ cảnh</p>)}
