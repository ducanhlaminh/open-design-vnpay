...
export function FeedbackSummaryView({ data, attachmentUrl }: FeedbackSummaryViewProps): JSX.Element {
  ...
  return <main className={styles.page}><header className={styles.header}><div><p className={styles.kicker}>Feedback</p><h1 className={styles.heading}>Tổng hợp phản hồi</h1></div><div className={styles.stats}><span><b>{visible.length}</b> bài gửi</span><span><b>{users.size}</b> người gửi</span><span><b>{latest ? new Date(latest).toLocaleString('vi-VN') : '—'}</b> gần nhất</span></div></header>{!data.storeReachable ? <div className={styles.warning}>Chưa kết nối media store — dữ liệu có thể thiếu</div> : null}<div className={styles.filters}><label className={styles.filter}>Workflow<select className={styles.select} value={workflow} onChange={(event) => setWorkflow(event.target.value)}><option value="">Tất cả workflow</option>{workflows.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label className={styles.toggle}><input type="checkbox" checked={hideDev} onChange={(event) => setHideDev(event.target.checked)} />Ẩn bản ghi dev <small className={styles.hint}>(mặc định bật — dữ liệu thử không làm bẩn số liệu thật)</small></label></div>{!visible.length ? <p className={styles.empty}>Chưa có bài gửi phù hợp. Hãy thử tắt bộ lọc.</p> : <div className={styles.versions}>{allAttachments.map(({ aggregate, attachments }) => <section className={styles.version} key={`${aggregate.form.version}-${aggregate.form.title}`}><h2 className={styles.versionTitle}>v{aggregate.form.version} · {aggregate.form.title} · {aggregate.submissionCount} bài gửi</h2>{aggregate.sections.map((section, index) => <section className={styles.section} key={`${section.title}-${index}`}>{section.title ? <h3 className={styles.sectionTitle}>{section.title}</h3> : null}{section.items.map((item) => <QuestionCard item={item} key={`${item.question.id}-${item.instanceLabel ?? ''}`} />)}</section>)}<Attachments attachments={attachments} attachmentUrl={attachmentUrl} /></section>)}</div>}</main>;
}
