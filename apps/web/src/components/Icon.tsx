...
export function Icon({ name, size = 14, strokeWidth = 1.6, ...rest }: Props) {
  ...
  switch (name) {
    ...
    case 'pipeline':
      // Two offset nodes linked by an elbow connector — reads as a branching
      // docs→UI flow (Lucide "workflow" silhouette). Distinct from `kanban`
      // (automations) and `orbit` at small rail sizes.
      return (
        <svg {...common}>
