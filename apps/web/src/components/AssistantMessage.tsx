...
import { SkillsAppliedCard } from "./SkillsAppliedCard";
...
interface Props {
  ...
  // Catalog used by SkillsAppliedCard to resolve display names for the
...
export function AssistantMessage({
  ...
  // Pull the first `skills_applied` event off the stream (synthesized
  // from the SSE start payload by providers/daemon.ts). Used to render
  // the "Skills applied" header card above the message content.
  const skillsApplied = events.find(
    (e): e is Extract<AgentEvent, { kind: 'skills_applied' }> =>
      e.kind === 'skills_applied',
  );
