 ...
 * goes through a typed ui_* tool that (a) scopes workspaceId to the ACTIVE
 * prototype workspace (switchable via ui_workspace_set_active; persisted in
 * OD_KG_ACTIVE), (b) stamps source:'agent' + updatedAt, and (c) refuses to
 * touch nodes that carry odClonedFrom or live in another workspace. The cloned
 * reference graph is therefore immutable to agents and
 * `tools-kg clone --refresh` is always safe.
 *
 * Workspace lifecycle: ui_workspace_{list,create,rename,set_active,delete}
 * manage the prototype workspaces an agent owns. Reference workspaces (holding
 * cloned UI_* nodes) are not agent-owned and cannot be renamed/deleted, and the
 * active workspace must be agent-owned so writes never land in reference data.
 */
...
export async function runMcpServer(config: KgConfig): Promise<void> {
  ...
  // Active prototype workspace. Mutable so `ui_workspace_set_active` can switch
  // it mid-session; every write tool reads `ws` at call time (closures), so a
  // switch takes effect immediately without restarting the server. The choice
  // is persisted to a singleton (OD_KG_ACTIVE) so it survives a restart; the
  // env-derived `config.prototypeWorkspaceId` is only the bootstrap default.
  let ws = config.prototypeWorkspaceId;
  const storedActive = await withSession(driver, (session) =>
    session.executeRead((tx) =>
      tx.run(
        `MATCH (st:OD_KG_ACTIVE {id: 'singleton'})
         OPTIONAL MATCH (w:UI_WORKSPACE {id: st.workspaceId})
         RETURN st.workspaceId AS wsId, w IS NOT NULL AS exists`,
      ),
    ),
  );
  const storedWsId = storedActive.records[0]?.get("wsId");
  if (storedWsId && storedActive.records[0]?.get("exists")) ws = storedWsId;

  /** Persist the active-workspace choice so it survives a server restart. */
  async function persistActiveWorkspace(workspaceId: string): Promise<void> {
    await withSession(driver, (session) =>
      session.executeWrite((tx) =>
        tx.run(
          `MERGE (st:OD_KG_ACTIVE {id: 'singleton'}) SET st.workspaceId = $workspaceId`,
          { workspaceId },
        ),
      ),
    );
  ...
  /* ── workspace lifecycle (CRUD) ────────────────────────────────────────── */

  server.registerTool(
    "ui_workspace_list",
    {
      description:
        "List every UI_WORKSPACE (agent prototype workspaces + reference workspaces holding cloned data), each with its screen count, whether it is agent-owned (deletable/renameable), and whether it is the currently active workspace that writes target. Read-only.",
      inputSchema: {},
    },
    async () => {
      const rows = await withSession(driver, (session) =>
        session.executeRead(async (tx) => {
          const result = await tx.run(
            `MATCH (w:UI_WORKSPACE)
             OPTIONAL MATCH (w)-[:OWNS_SCREEN]->(s:UI_PROJECT_SCREEN)
             RETURN w.id AS id, w.name AS name, w.source AS source,
                    count(s) AS screenCount
             ORDER BY w.id`,
          );
          return result.records.map((r) => {
            ...
            return {
              ...
              agentOwned: r.get("source") === "agent",
              active: id === ws,
              screenCount: Number(r.get("screenCount") ?? 0),
            };
          });
        }),
      );
      return text({ activeWorkspace: ws, rowCount: rows.length, workspaces: rows });
    },
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Create (or update the name of) an agent-owned prototype workspace. id is the kebab-case workspace slug. Optionally set it active so subsequent screen/instance writes land in it. Use this in the react-shadcn Step 0 'new project' branch instead of editing OD_KG_WORKSPACE + restarting.",
      inputSchema: {
        id: z.string().regex(/^[a-z0-9][a-z0-9-]*$/).describe("kebab-case workspace id, e.g. ws-checkout-redesign"),
        name: z.string().min(1).describe("human-readable workspace name"),
        setActive: z.boolean().optional().describe("switch the active workspace to this one (default false)"),
      },
    },
    async ({ id, name, setActive }) => {
      const existingSource = await withSession(driver, (session) =>
        session.executeRead(async (tx) => {
          const result = await tx.run(
            `MATCH (w:UI_WORKSPACE {id: $id}) RETURN w.source AS source`,
            { id },
          );
          return result.records.length === 0 ? undefined : (result.records[0].get("source") ?? null);
        }),
      );
      if (existingSource !== undefined && existingSource !== "agent") {
        throw new Error(`workspace id "${id}" already exists and is not agent-owned (reference data) — pick another id`);
      }
      const row = await withSession(driver, (session) =>
        session.executeWrite(async (tx) => {
          const result = await tx.run(
            `MERGE (w:UI_WORKSPACE {id: $id})
               ON CREATE SET w.createdAt = $now, w.source = 'agent'
             SET w.name = $name, w.updatedAt = $now
          ...
          return {
            ...
            created: existingSource === undefined,
          };
        }),
      );
      if (setActive) {
        ws = row.id;
        await persistActiveWorkspace(ws);
      }
      return text({ ...row, active: ws === row.id, activeWorkspace: ws });
    },
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Rename an agent-owned prototype workspace (changes the display name only — the id/slug is the stable key for screens and stays fixed). Refuses reference workspaces.",
      inputSchema: {
        id: z.string().min(1),
        name: z.string().min(1),
      },
    ...
    async ({ id, name }) => {
      const row = await withSession(driver, (session) =>
        session.executeWrite(async (tx) => {
          const result = await tx.run(
            `MATCH (w:UI_WORKSPACE {id: $id}) WHERE w.source = 'agent'
             SET w.name = $name, w.updatedAt = $now
             RETURN w.id AS id, w.name AS name`,
            { id, name, now: new Date().toISOString() },
          );
      ...
      if (!row) throw new Error(`workspace "${id}" not found or not agent-owned (reference workspaces cannot be renamed)`);
      return text(row);
    },
  );

  server.registerTool(
    "ui_workspace_set_active",
    {
      description:
        "Switch the active prototype workspace that screen/instance/flow writes target. Takes effect immediately (no restart) and persists across restarts. The target must be an agent-owned workspace so writes never land in cloned reference data.",
      inputSchema: { id: z.string().min(1) },
    },
    async ({ id }) => {
      const ok = await withSession(driver, (session) =>
        session.executeRead(async (tx) => {
          const result = await tx.run(
            `MATCH (w:UI_WORKSPACE {id: $id}) RETURN w.source AS source`,
            { id },
          );
          return result.records[0]?.get("source") ?? null;
        }),
      );
      if (ok === null) throw new Error(`workspace "${id}" does not exist — create it with ui_workspace_create first`);
      if (ok !== "agent") throw new Error(`workspace "${id}" is a reference workspace, not agent-owned — cannot make it the active write target`);
      ws = id;
      await persistActiveWorkspace(ws);
      return text({ activeWorkspace: ws });
    },
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Delete an agent-owned prototype workspace and all its screens/instances. Guardrails: refuses reference (non-agent) workspaces, refuses the currently active workspace (switch away first), refuses if the workspace contains any cloned node (odClonedFrom). If the workspace still has screens, pass force:true to cascade-delete them.",
      inputSchema: {
        id: z.string().min(1),
        force: z.boolean().optional().describe("required when the workspace still contains screens; cascade-deletes the whole subtree"),
      },
    },
    async ({ id, force }) => {
      if (id === ws) {
        throw new Error(`workspace "${id}" is the active workspace — switch to another with ui_workspace_set_active before deleting`);
      }
      // Inspect first so each refusal reason is specific.
      const info = await withSession(driver, (session) =>
        session.executeRead(async (tx) => {
          const result = await tx.run(
            `MATCH (w:UI_WORKSPACE {id: $id})
             OPTIONAL MATCH (w)-[:OWNS_SCREEN]->(s:UI_PROJECT_SCREEN)
             OPTIONAL MATCH (n {workspaceId: $id})
             RETURN w.source AS source,
                    count(DISTINCT s) AS screenCount,
                    count(DISTINCT CASE WHEN n.odClonedFrom IS NOT NULL THEN n END) AS clonedCount`,
            { id },
          );
          const rec = result.records[0];
          if (!rec || rec.get("source") === null) return null;
          return {
            source: rec.get("source"),
            screenCount: Number(rec.get("screenCount") ?? 0),
            clonedCount: Number(rec.get("clonedCount") ?? 0),
          };
        }),
      );
      if (!info) throw new Error(`workspace "${id}" does not exist`);
      if (info.source !== "agent") throw new Error(`workspace "${id}" is a reference workspace, not agent-owned — refusing to delete`);
      if (info.clonedCount > 0) throw new Error(`workspace "${id}" contains ${info.clonedCount} cloned reference node(s) — refusing to delete`);
      if (info.screenCount > 0 && !force) {
        throw new Error(`workspace "${id}" still has ${info.screenCount} screen(s) — pass force:true to cascade-delete them`);
      }
      const deleted = await withSession(driver, (session) =>
        session.executeWrite(async (tx) => {
          // Delete every agent-owned node scoped to this workspace, then the
          // workspace node itself. Re-assert no cloned node slipped in.
          const result = await tx.run(
            `MATCH (w:UI_WORKSPACE {id: $id}) WHERE w.source = 'agent'
             OPTIONAL MATCH (n {workspaceId: $id})
             WITH w, collect(DISTINCT n) AS owned
             WHERE all(x IN owned WHERE x IS NULL OR x.odClonedFrom IS NULL)
             FOREACH (x IN owned | DETACH DELETE x)
             DETACH DELETE w
             RETURN size([x IN owned WHERE x IS NOT NULL]) AS nodesDeleted`,
            { id },
          );
          if (result.records.length === 0) throw new Error(`workspace "${id}" delete aborted — a cloned node was present`);
          return Number(result.records[0].get("nodesDeleted") ?? 0);
        }),
      );
      return text({ id, deleted: true, nodesDeleted: deleted, screensDeleted: info.screenCount });
    },
  );
