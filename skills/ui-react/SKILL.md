...
### 1a. Apply Design System review rules when staged
If `./criteria/rules.md` exists, read it before writing screens. It may be hand-authored OR auto-generated by the daemon from the DS's showcase + tokens, so apply whatever it contains — overlays, feedback, tables, badges, primary actions, layout, states, plus color/typography/spacing/elevation/radius. Preserve the intended decision from UX Spec and record deviations in a code comment (citing the **exact anchor written in the file**) when product docs win. If `./criteria/components.md` exists, treat it as the closed valid component catalogue for component naming and intent. If a component entry in `criteria/components.md` has no "Mô tả:" line, look up `criteria/components-guide.md` (same directory, when present) at the matching anchor `#figma-…`: it's an AI-generated description derived from the node + Figma screenshot analysis — use it as **context only** to understand the component, never as the anchor itself (that's still `criteria/components.md#…`), and cite it with the source tag "(AI sinh)" when quoted in a code comment. Missing `components-guide.md` or no matching entry means treat the component as having no description, same as before. Missing criteria files are valid.

### 1b. The wireframe IS the layout contract — do NOT re-derive it
The `ux` stage emits `./wireframes/<SCREEN-ID>.html` for every screen. Each
wireframe is a self-contained HTML document: its DOM, nested blocks, semantic
markup, and CSS are the layout contract. Read it for each screen and honor it:

- **Structure**: preserve the HTML DOM composition — block nesting, section
  order, grouping, side-by-side placement, and primary-action placement. If you
  deviate, it must be a deliberate craft improvement, not because you skipped
  reading the file.
- **Components**: read `data-comp` on each component block. This stage has no
  Design System; treat the value as a role hint and choose the appropriate
  shadcn primitive or composition for the described block. There is no closed
  slug mapping table.
- **Navigation and variants**: `data-nav` is the destination screen ID and must
  match the `data-flow-action` / route you wire in step 4. `data-variant` carries
  the component variant intent.
- **Responsive contract**: the `@media` rules in the HTML, at 834px and 390px
  where present, are the web screen's responsive redesigns — see the breakpoint
  rules below.
...
- **Web screens MUST be responsive** — the preview shows every `layout: "web"`
  ...
  - When the UX Spec's wireframe carries per-device redesigns
    (`wireframes/<id>.html` → `@media breakpoints at 834px and 390px`), those
