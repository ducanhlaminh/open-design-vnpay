...
When present, read `./criteria/rules.md` and `./criteria/components.md` before authoring the prototype. `rules.md` may be hand-authored OR auto-generated by the daemon from the DS's showcase + tokens, so apply whatever it contains — component choice, overlays, feedback, tables, badges, actions, plus color/typography/spacing/elevation/radius. Keep a traceable rule ID (the **exact anchor written in this file**, e.g. `R-SPACING-BASE`) in implementation notes or nearby markup comments when a rule drives a choice; do not assume a fixed anchor set. `components.md` is the closed valid component catalogue; do not invent DS components. If a component entry in `components.md` has no "Mô tả:" line, look up `./criteria/components-guide.md` (same directory, when present) at the matching anchor `#figma-…`: an AI-generated description from node + Figma screenshot analysis — use it as **context only**, never as the traceable anchor itself (still only `criteria/components.md#…` / `criteria/rules.md#…`), and cite it with the source tag "(AI sinh)" when quoted in a markup comment. Missing file or no matching entry means treat the component as having no description, same as before. Missing files are valid; use the normal skill defaults.
