...
export type PersistedAgentEvent =
  ...
  | {
      /**
       * Synthesized by the web client when an SSE `start` payload carries
       * skill ids. Surfaces what context the daemon injected into the
       * system prompt for the upcoming turn so the chat UI can render an
       * "Applied skills" card above the assistant message. Currently
       * web-only (not persisted); reload after a run will not replay this
       * event until the daemon mirrors it through the agent channel.
       */
      kind: 'skills_applied';
      skillId?: string | null;
      skillIds?: string[];
