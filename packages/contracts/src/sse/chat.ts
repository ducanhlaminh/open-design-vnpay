...
export interface ChatSseStartPayload {
  ...
  /**
   * Skills active for this run. `skillId` is the primary project-persistent
   * skill resolved at spawn time (may be inherited from project default).
   * `skillIds` are the per-turn @-mention skills concatenated for this run
   * only. Both arrive as canonical ids; web resolves display metadata from
   * the registry cache. Surfacing them lets the chat UI render an
   * "Applied skills" card above the assistant message so the user can see
   * which prompt context was in play for that turn.
   */
  skillId?: string | null;
  skillIds?: string[];
  /**
